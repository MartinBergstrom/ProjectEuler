...
//How can you optimize? Scanner takes time? doesn't matter if skip 0's
public class LargestProductInASeriesOptimized {
	...
	public LargestProductInASeriesOptimized(){
		...
		try {
			...
			input.useDelimiter("");
				...
				while(input.hasNext()){
					char nextChar = input.next().charAt(0);
					//detta kan bli fel? listan innehåller ej adjacent
					if(!Character.isWhitespace(nextChar) &&nextChar!= 0){
